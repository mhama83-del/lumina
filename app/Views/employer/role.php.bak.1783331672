    ...
    <form method="get" action="<?= base_url('employer/compare') ?>" id="cmpForm">
      ...
      <div class="stack">
        <?php foreach ($ranked as $i => $c):
          ...
          $body = '<p class="muted">'.esc($c['explanation']).'</p>'
                . '<div class="section-label" style="margin-top:10px">Sub-scores</div>'
                . '<p style="font-size:13px">Skill '.$c['skill_match_score'].'% · Evidence '.$c['evidence_strength_score'].' · Velocity '.$c['learning_velocity_score'].' · Animal '.$c['animal_fit_score'].' · Domain '.$c['domain_fit_score'].' · CGPA '.$c['academic_fit_score'].'</p>'
                . ($c['skill_overlap'] ? '<div class="section-label" style="margin-top:8px">Skill overlap</div><p style="font-size:13px">'.esc(implode(', ',$c['skill_overlap'])).'</p>' : '')
                . ($c['missing_skills'] ? '<div class="section-label" style="margin-top:8px">Missing</div><p style="font-size:13px">'.esc(implode(', ',$c['missing_skills'])).'</p>' : '')
